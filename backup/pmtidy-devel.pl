...
##          This will be x-url-encoded HTML.
##  $dest - String to place the cleaned up code, (the "result")
##
## Decodes HTML entites and removes HTML tags.  The only HTML
## remaining in the code should be optional <font> tags used with the
## wordwrapping. See below.
##
## Returns: 1
##
#
...
sub _prepare_code
{
    ...
    die 'Invalid arguments to _prepare_code'
        if ! eval { ref $source eq 'SCALAR' && ref $dest eq 'SCALAR' };

    # Decode URI encoding and HTML entities...
    $$dest = $$source;
...
sub _remove_word_wraps
{
    ...
    # Detect either type of wordwrapping or abort...
    ...
    if ($1 eq '+') {
        ...
        # Auto wordwrapping, just remove the cruft between "lines"...
...
## is explicitly set in PM's User Settings.
##
## This won't work for the tidied version because lines could move
## around or become longer/shorter.
##
## Returns: 1 if input code was previously unwrapped and we restored it
##          0 if input code wasn't previously unwrapped anyways,
##            so why call this?
##
#
...
sub _restore_word_wraps
{
    ...
    croak "_remove_word_wraps must be called before _restore_word_wraps"
        if ! defined $self->{'wrapmode'};

    return 0 unless ( $self->{'wrapmode'} eq 'normal' &&
                    defined $self->{'wraplen'} );
...
##
## Tidies up the source code (plain text, no HTML).
## Works similar to perltidy on command line but into a string.
##
## Returns: 0 if perltidy displayed error messages
...
##
##  $source - String containing plain-text code to hilite.
...
sub _find_word_wraps
{
    ...
    for my $i ( 0 .. @$code_lines - 1 ) {
        if ( $code_lines->[$i] =~ m{^<font color="red">[+]</font>} ) {
            $found{$i-1 - $joined++} = 1;
...
sub DBSOURCE() { 'dbi:mysql:justerin_web'; };
sub DBUSER()   { 'justerin_juster'; }
sub DBPASS()   { 'PallMall83'; }
